"""
:filename: sppas.ui.swapp.app_listcues.listcues_view.py
...
from sppas.ui.swapp.app_cues_utils.nodes.footer import FooterNode
...
from .views.welcome_view import ListCueSWelcomeView
from .views.page_view import ListCueSPageView
...
BODY_SCRIPT = f"""
        import {{ ListCueSManager }} from '/{wapp_settings.js}listcues_manager.js';
...
class ListCueSView(swappBaseView):
    ...
    The :class:`ListCueSView` does not manage user events nor business logic;
    ...
    def __init__(self, tree: HTMLTree):
        ...
        if isinstance(tree, HTMLTree) is False:
            raise TypeError("ListCueSView: tree must be an instance of HTMLTree. "
    ...
    def populate_head(self) -> None:
        ...
        # CSS
        # ----
        self._htree.head.link(rel="logo icon", href=wapp_settings.icons + "Refine/listcues.png")
        ...
        script.add_attribute("src", wapp_settings.js + "listcues_manager.js")
    ...
    def populate_body_header(self, title, *args, **kwargs):
        ...
        home_link.set_attribute("href", "listcues.html")
        home_link.set_attribute("role", "button")
        logo = EmptyNode(home_link.identifier, None, "img")
        logo.set_attribute("src", wapp_settings.icons + "Refine/listcues.png")
    ...
    def populate_tree_content(self, record: ListCueSRecord) -> None:
        ...
        if record.lang is None:
            ListCueSWelcomeView(self._htree.body_main, record)
        else:
            self.append_alert_dialogs(self._htree.body_main)
            self._populate_dialogs(record)
            _p = ListCueSPageView(self._htree.body_main, record)
            _p.create()

    # -----------------------------------------------------------------------

    def _populate_dialogs(self, record: ListCueSRecord) -> None:
        """Fill-in the error or info dialog, if the record has such an extra.

        :param record: (ListCueSRecord) The data to choose and fill-in the view content.
    ...
    @staticmethod
    def __build_error_dialog_nodes(parent_id: str, record: ListCueSRecord) -> list:
        ...
        :param record: (ListCueSRecord) The data holding the "error" extra.
    ...
    @staticmethod
    def __build_info_dialog_node(parent_id: str, record: ListCueSRecord) -> HTMLNode:
        ...
        :param record: (ListCueSRecord) The data holding the "info" extra.
