"""
:filename: sppas.ui.swapp.app_twincues.twincuesmaker.py
...
class TwinCueSResponseRecipe(swappBaseResponse):
    ...
    def _process_events(self, events: dict, **kwargs) -> bool:
        ...
        """
        logging.debug(f" >>>>> Page Application TwinCueS -- Process events: {events} <<<<<< ")
        self._data = dict()
        ...
        # form ("lang" + "word"/"cue", a plain POST) are real page
        # navigations, parsed into "events" the very same way (see
        # WhakerPy's process_post() and parse_query_string()). The whole
        # page must be re-created (True) so that _bake() renders the
        # conversion content, including the Yoyo dialogs for any error or info.
        #
        # Except on the fixed "twincues.html" itself: a bot only knows that
        ...
        # header logo link back to "twincues.html" -- or a "lang" query
        ...
        # previous request left it in, instead of going back to the
        # welcome page.
