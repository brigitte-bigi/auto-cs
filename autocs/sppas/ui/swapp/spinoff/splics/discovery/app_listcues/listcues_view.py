"""
:filename: sppas.ui.swapp.spinoff.splics.discovery.app_listcues.listcues_view.py
:author: Brigitte Bigi
:contact: contact@sppas.org
:summary: Main view of "ListCueS" app.
...
from .listcues_msg import MSG_APP_TITLE
from .listcues_msg import MSG_HOME
from .listcues_msg import MSG_APP_SUBTITLE
from .listcues_msg import MSG_ACS_PROJECT
from .listcues_msg import MSG_ERROR_DETAILS
from .views.welcome_view import splicsListCueSWelcomeView
from .views.page_view import splicsListCueSPageView
...
BODY_SCRIPT = f"""
        import {{ ListCueSManager }} from '/{splics_paths.js}listcues_manager.js';
        const listcuesManager = new ListCueSManager();
        listcuesManager.handleCuesManagerOnLoad();
"""

# ---------------------------------------------------------------------------


class splicsListCueSView(swappBaseView):
    """View class is responsible for populating the *listcues*.html* pages.

    This class represents the **View** component of the MVC pattern for the
    ListCueS web application. It receives an existing :class:`HTMLTree`
    instance and fills it with all static and dynamic visual content.

    The :class:`splicsListCueSView` does not manage user events nor business logic;
    it focuses solely on defining the HTML structure and resources required
    for rendering the ListCueS conversion interface.

    """

    def __init__(self, tree: HTMLTree):
        """Initialize and populate the ListCueS view structure.
        ...
        if isinstance(tree, HTMLTree) is False:
            raise TypeError("splicsListCueSView: tree must be an instance of HTMLTree. "
    ...
    def populate_head(self) -> None:
        ...
        # CSS
        # ----
        self._htree.head.link(rel="logo icon", href=splics_paths.logos + "listcues.png")
        ...
        script.add_attribute("src", splics_paths.js + "listcues_manager.js")
    ...
    def populate_body_header(self, title, *args, **kwargs):
        ...
        _h2 = HTMLNode(self._htree.body_header.identifier, None, "h2", value=MSG_APP_SUBTITLE)
    ...
    def _populate_body_nav(self, *args, **kwargs):
        ...
        _app = splicsMenuLinkButtonNode(_s.identifier, "link-app_button", "listcues.html")
        _app.add_attribute("data-target", "_self")
        _app.set_icon(None, splics_paths.logos + "listcues.png")
    ...
    def populate_tree_content(self, record: splicsListCueSRecord) -> None:
        ...
        the controller shows the conversion content (key piano and result).

        :param record: (splicsListCueSRecord) The data to fill-in the view content.

        """
        if record.lang is None:
            splicsListCueSWelcomeView(self._htree.body_main, record)
        else:
            self.append_alert_dialogs(self._htree.body_main)
            self._populate_dialogs(record)
            _p = splicsListCueSPageView(self._htree.body_main, record)
            _p.create()

    # -----------------------------------------------------------------------

    def _populate_dialogs(self, record: splicsListCueSRecord) -> None:
        """Fill-in the error or info dialog, if the record has such an extra.

        :param record: (splicsListCueSRecord) The data to choose and fill-in the view content.
    ...
    @staticmethod
    def __build_error_dialog_nodes(parent_id: str, record: splicsListCueSRecord) -> list:
        ...
        :param record: (splicsListCueSRecord) The data holding the "error" extra.
    ...
    @staticmethod
    def __build_info_dialog_node(parent_id: str, record: splicsListCueSRecord) -> HTMLNode:
        ...
        :param record: (splicsListCueSRecord) The data holding the "info" extra.
