...
- **splicsTwinCueSResponseRecipe** is the single *HTTP transport layer* recipe,
...
- **splicsTwinCueSController** represents the *MVC controller*.
...
- **splicsTwinCueSView** is the *View* component responsible for building the static
  and dynamic HTML structure (head, header, body, footer, scripts) using
  WhakerPy's HTMLTree utilities. It shows **splicsTwinCueSWelcomeView** while
  `record.lang` is `None`, and **splicsTwinCueSPageView** (word/cue textareas,
  piano and result) once a language is known.

- **splicsTwinCueSModel** dispatches to two conversion models:
  **splicsWord2CuesModel** (word -> cue) and **splicsCues2WordsModel** (cue -> word(s)).
