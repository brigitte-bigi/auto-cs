...
class SPPASAnnotateNode(BaseViewNode):
    ...
    def _lang_choices(self, languages: dict):
        ...
        """
        label = HTMLNode(self._form.identifier, None, "label", attributes={"for": "lang"},
                         value=SPPASAnnotateNode.MSG_LANG)
        self._form.append_child(label)
        div = HTMLNode(self._form.identifier, None, "div", attributes={"class": "select width-half"})
        self._form.append_child(div)
        select = HTMLNode(div.identifier, None, "select", attributes={"id": "lang", "name": "lang"})
